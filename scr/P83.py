...
def P83(N=20): 
	...
	right, up, and down in matrix.txt (right click and "Save Link/Target As..."), a 31K 
	...
	wait_list, checked, suma, Vmin = search( [ [0, 0, weith_matrix[0,0]] ], [ [0,0] ], 0 )
	while len(wait_list) > 0:
		distance_matrix[Vmin[0], Vmin[1]]  = suma
		wait_list, checked, suma, Vmin = search( wait_list, checked, suma )

	distance_matrix[Vmin[0], Vmin[1]]  = suma

	print( distance_matrix )
	print( np.min(distance_matrix[:, -1]) )

	return np.min(distance_matrix[:, -1])
