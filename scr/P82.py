...
def P81(N=20): 
	...
	wait_list, checked, suma, Vmin = search( [ [h, 0, weith_matrix[h,0]] for h in range(wm_h) ], [ [h,0] for h in range(wm_h) ], 0, mov=[[1,0], [0,1], [-1,0]] )
	while len(wait_list) > 0:
		distance_matrix[Vmin[0], Vmin[1]]  = suma
		wait_list, checked, suma, Vmin = search( wait_list, checked, suma, mov=[[1,0], [0,1], [-1,0]] )
