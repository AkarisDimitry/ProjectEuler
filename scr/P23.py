...
def P23(N=30000, v=False): 
	...
	can be shown that all integers greater than 28123 can be written as the sum of two abundant 
	...
	an = abundant_numbers(N) 

	suma = 0
	for n in range( N ):

		if not np.isin(n-an, an).any() :
			suma += n

		if n%500==0: print(n, suma)
